...
namespace AgraBot
{
    static class Program
    {
        ...
        /// </summary>
        static readonly Mutex Mutex = new Mutex(true, "{8F6F0AC5-B9A1-55fd-A8CF-82F04E6BDE8F}");
        [STAThread]
        static void Main()
        {
            ...
            {
                //opening the subkey  
                RegistryKey regKey = Registry.CurrentUser.OpenSubKey(@"SOFTWARE\AgraBot");

                //create default keys if not existing
                if (regKey == null)
                {
                    RegistryKey Key = Registry.CurrentUser.CreateSubKey(@"SOFTWARE\AgraBot");
            ...
            {
                MessageBox.Show("AgraBot is Already Running");
            }
        }
