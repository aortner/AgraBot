...
namespace AgOpenGPS
{
    ...
    public class CBoundaryLines
    {
        ...
        //constructor
        public CBoundaryLines(OpenGL _gl, OpenGL _glb, FormGPS _f)
        {
            mf = _f;
            gl = _gl;
            glb = _glb;
        ...
        public void DrawBoundaryLine()
        {
            ...
            if (ptCount < 1) return;
            gl.LineWidth(2);
            if (isDriveThru) gl.Color(0.25f, 0.952f, 0.960f);
            else gl.Color(0.95f, 0.2f, 0.860f);
            gl.Begin(OpenGL.GL_LINE_STRIP);
            for (int h = 0; h < ptCount; h++) gl.Vertex(bndLine[h].easting, bndLine[h].northing, 0);
            gl.End();
            ...
            gl.Begin(OpenGL.GL_LINE_STRIP);
            gl.Vertex(bndLine[ptCount - 1].easting, bndLine[ptCount - 1].northing, 0);
            gl.Vertex(bndLine[0].easting, bndLine[0].northing, 0);
            gl.End();
